	...
	, mSprites(static_cast<size_t>(Textures::_Count))
{
	...
	mTextures[(size_t)Textures::DeadGhost].loadFromFile(folderStr + "\\dead-ghost.png");
	mSprites[(size_t)Textures::DeadGhost].setTexture(mTextures[(size_t)Textures::DeadGhost]);
	...
	if (scaleOnLoad)
	{
		...
		mSprites[(size_t)Textures::DeadGhost].scale(0.08f, 0.08f);
